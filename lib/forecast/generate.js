...
function deriveDay(date, index, hours, periods, afd) {
  ...
  const story = language({ storm, severe, maxPop, avgCloud, high, bestWindow, index, date });
  return {
...
function language(x) {
  if (x.storm) {
    const stories = [
      ...
      { headline:"Typical July Heat With Late Storms", narrative:"A familiar midsummer pattern continues: warm sunshine early, then building clouds and a late-day storm chance. Timing is less certain this far out, but the morning currently looks quieter." }
    ];
  ...
  if (x.avgCloud >= 65) return { headline: "Cloudy Start, Better by Afternoon", narrative: "Clouds may hang on for a while, especially near the higher terrain, before a few brighter breaks develop. Most of the day stays dry and manageable." };
  return { headline: x.high >= 88 ? "Typical July Heat With Plenty of Sun" : "A Quieter Day Takes Shape", narrative: x.high >= 88 ? "Sunshine will push temperatures higher through the afternoon. Rain disruption looks limited, with the heat and humidity becoming the main story." : "A mainly dry setup brings a useful stretch of outdoor weather. Clouds will be limited enough for sunshine without a meaningful rain interruption." };
}
