...
const FALL_MILESTONE_KEY_PREFIX = "fall:milestones:v1";
...
export async function loadFallMilestoneArchive(season) {
  if (!hasKvConfiguration() || !Number.isInteger(Number(season))) return null;
  try {
    const { kv } = await import("@vercel/kv");
    return await kv.get(`${FALL_MILESTONE_KEY_PREFIX}:${season}`);
  } catch (error) {
    console.warn("Fall milestone KV read unavailable:", error.message);
    return null;
  }
}

export async function saveFallMilestoneArchive(archive) {
  if (!hasKvConfiguration() || !Number.isInteger(Number(archive?.season))) return false;
  try {
    const { kv } = await import("@vercel/kv");
    await kv.set(`${FALL_MILESTONE_KEY_PREFIX}:${archive.season}`, archive);
    return true;
  } catch (error) {
    console.warn("Fall milestone KV write unavailable:", error.message);
    return false;
  }
}
