...
export function buildStormSignal({ radar = {}, sky = {}, weather = {} } = {}) {
  ...
  const radarOrganizedNear = radarPresent && (number(radar.nearWestEchoPixels) ?? 0) >= 25;
  ...
  const thunderstormConfirmed = lightning;
  const radarDevelopmentSupport = thunderstormConfirmed &&
    radarApproaching &&
    (radarStrong || (radarGrowing && radarOrganizedNear));
  const evidenceGate = radarPresent && radarDevelopmentSupport && skySupport && (weatherSupport || lightning);
