...
test("uses developing wording for a lightning-confirmed approaching storm", () => {
  const result = buildStormSignal({
    radar: {
      available: true,
      ageMinutes: 2,
    ...
    weather: { tempF: 82, dewF: 64, humidity: 61, lightningCount: 1 }
  });

  assert.equal(result.active, true);
  assert.equal(result.level, "developing");
  assert.equal(result.headline, "Storm clouds may be developing west of Asheville.");
});

test("rejects strong approaching rain echoes without lightning confirmation", () => {
  const result = buildStormSignal({
    radar: {
      available: true,
      ageMinutes: 1,
      echoCoverage: 0.15,
      echoPixels: 1500,
      nearWestEchoPixels: 300,
      strongEchoPixels: 90,
      nearestEchoMiles: 10,
      growing: true,
      approaching: true
    },
    sky: { mode: "day", cloudCoverWest: 80, cloudTrend: "increasing" },
    weather: { tempF: 84, dewF: 70, humidity: 72, lightningCount: 0 }
  });

  assert.equal(result.active, false);
  assert.equal(result.inputs.thunderstormConfirmed, false);
  assert.equal(result.headline, null);
});
...
test("uses stronger plain language when multiple signals agree", () => {
  const result = buildStormSignal({
    ...
    weather: { tempF: 86, dewF: 70, humidity: 66, lightningCount: 2 }
  });
