...
function analyzeFrame({ width, height, data }, { skyCrop = 0.2 } = {}) {
  ...
  const pixelClassAt = (x, y) => {
    ...
    return { brightness, saturation, blue, gray };
  };

  for (let y = 0; y < height; y += step) for (let x = 0; x < width; x += step) {
    const { brightness, saturation, blue, gray } = pixelClassAt(x, y);
    if (y < skyBottom) {
  ...
  for (let y = step; y < skyBottom - step; y += step) for (let x = step; x < width - step; x += step) {
    // The courthouse feed stamps a high-contrast label into the upper-left.
    // Keep the original, larger exclusion and only count edges whose pixels
    // already look like sky/cloud. Otherwise text, ridges, trees, and JPEG
    // noise can be promoted into fabricated cloud structure.
    if (x < width * 0.32 && y < skyBottom * 0.45) continue;
    const centerClass = pixelClassAt(x, y);
    const rightClass = pixelClassAt(x + step, y);
    const downClass = pixelClassAt(x, y + step);
    const skyLike = centerClass.blue || centerClass.gray;
    const neighborSkyLike = rightClass.blue || rightClass.gray || downClass.blue || downClass.gray;
    if (!skyLike || !neighborSkyLike) continue;
    const center = luminanceAt(x, y);
    const gradient = (Math.abs(center - luminanceAt(x + step, y)) + Math.abs(center - luminanceAt(x, y + step))) / 2;
    const cloudEdge = centerClass.gray || rightClass.gray || downClass.gray;
    structureSamples++; gradientTotal += gradient; if (gradient >= 12 && cloudEdge) structuredSamples++;
  }
  ...
  // Color-classified cloud pixels are the coverage measurement. Structure is
  // supporting evidence for cloud type/texture, never a second coverage model.
  const cloudCover = rawCloud;
  const buildingCloudStructureSignal = rawCloud >= 12 && blueShare < 0.85 && structureScore >= 0.09;
  ...
  return {
    ...
    buildingCloudStructureSignal, buildingCloudStructureScore: +structureScore.toFixed(2), structureCloudEstimate: structureEstimate,
...
function inferCloudTypes(metrics) {
  ...
  if (metrics.buildingCloudStructureSignal && metrics.buildingCloudStructureScore >= 0.2) return [{ type: "towering_cumulus", confidence: 0.72 }];
  if (metrics.buildingCloudStructureSignal && metrics.buildingCloudStructureScore >= 0.11 && metrics.cloudCoverWest >= 55) return [{ type: "stratocumulus", confidence: 0.67 }];
...
async function buildPayload() {
  ...
  const usable = successful.filter((entry) => !["offline", "night", "stale", "obscured"].includes(entry.observation.quality));
  const downtown = usable.find((entry) => entry.camera.id === "downtown-asheville-west"), north = usable.find((entry) => entry.camera.id === "north-asheville-south"), primary = downtown || north;
  ...
  const metrics = primary
    ? { ...primary.metrics, cloudCoverWest: downtown?.metrics.cloudCoverWest ?? (Number.isFinite(northWest) ? northWest * 100 : primary.metrics.cloudCoverWest), cloudCoverENE: Number.isFinite(northEast) ? northEast * 100 : null }
    : { cloudCoverWest: null, cloudCoverENE: null, sunlightDetected: false, sunlightLevel: "weak", mode: "unavailable", qualityFlags: ["no-fresh-camera"] };
  const captureTimestamp = primary?.observation.timestamp || successful[0]?.observation.timestamp || new Date().toISOString();
  return {
    timestamp: captureTimestamp, captureStatus: usable.length ? (failed.length ? "partial" : "live") : "stale", metrics, observations,
