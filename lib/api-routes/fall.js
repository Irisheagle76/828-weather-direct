...
import { loadPublicFallMilestones } from "../fall/milestone-service.js";
...
export async function buildPayload({ now = Date.now(), fetcher = fetch, observationUrl = null, observationLoader = null, observationPayload = null } = {}) {
  ...
  const milestones = await loadPublicFallMilestones(now);
  ...
  return {
    ...
    calibration,
    milestones,
    peakTiming: seasonalPeakTimingInput(now),
    quality: {
...
function seasonalPeakTimingInput(now) {
  const value = Number(process.env.FALL_SEPTEMBER_MEAN_F);
  const valid = Number.isFinite(value) && value >= 45 && value <= 85;
  return {
    seasonYear: new Date(now).getFullYear(),
    septemberMeanF: valid ? value : null,
    source: valid ? (process.env.FALL_SEPTEMBER_MEAN_SOURCE || "Seasonal September mean input") : null
  };
}
