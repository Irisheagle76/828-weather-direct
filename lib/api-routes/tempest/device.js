  ...
  try {
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (obs) {
          station_observation = normalizeTempestDeviceObservation(obs);
          station_observation = {
            ...station_observation,
            pressure: null
          };
...
function normalizeCurrent(c) {
  return {
    ...
    lightningStrikeDistance:
      ...
      null,
    lightningStrikeLastAt: normalizeTs(
      c.lightning_strike_last_epoch ??
      c.lightningStrikeLastAt ??
      c.lightning_strike_last_at
    ),
...
function normalizeObsObject(obs = {}) {
  ...
  return {
    ...
    lightningStrikeCount:
      ...
      0,
    lightningStrikeLastAt: normalizeTs(
      obs.lightningStrikeLastAt ??
      obs.lightning_strike_last_epoch ??
      obs.lightning_strike_last_at
    ),
    uv: obs.uv ?? null,
