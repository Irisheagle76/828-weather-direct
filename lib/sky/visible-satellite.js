...
export function parseVisibleImages(html = "") {
  ...
  const candidates = [
    ...
    ...source.matchAll(/["']([^"']*\/data\/satellite\/\d{8}\/CLT\/color\/\d{8}_\d{6}_CLT\.jpg)["']/gi)
  ].map((match) => match[1]);
  const urls = candidates.flatMap((candidate) => {
    const imageUrl = new URL(candidate, NCAR_PAGE_URL);
...
export function analyzeVisibleSatelliteFrame(frame, { observedAt = null, now = Date.now(), previous = null } = {}) {
  ...
  // A previous strong signature can inform the trend, but it must not keep the
  // current frame in the definitive "likely" category after the valley signal
  // has weakened. Surface observations and cameras decide whether a current
  // satellite signature can be promoted beyond "possible".
  const valleyPattern = valleyFogScore >= 0.52 ? "likely" : valleyFogScore >= 0.12 ? "possible" : "none";
