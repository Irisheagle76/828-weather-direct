...
// ============================================================
// TODAY OUTLOOK (FINAL CLEAN VERSION)
// ============================================================
export function renderTodayOutlook(intel) {
  const emojiEl = document.getElementById("today-emoji");
  const headlineEl = document.getElementById("today-headline");
  ...
  // 🌙 SWITCH
  if (hour >= 15) {
    const stats = remainder?.statsRemainder || intel.current;

    action = generateHumanAction({
      temp: stats.avgTemp ?? stats.temp,
      feelsLike: stats.avgTemp ?? stats.temp,
      dewpoint: stats.avgDew ?? stats.dewpoint,
      humidity: stats.avgHumidity ?? stats.humidity,
      windSpeed: stats.avgWind ?? stats.windSpeed,
      windGust: stats.maxWindGust ?? stats.windGust,
      precipType: "none",
      precipIntensity: 0,
      uvIndex: 0,
      visibility: 10,
  ...
  // 🌙 EMOJI SYNC
  if (hour >= 15 && future?.length) {
    action.emoji = future[0].emoji || action.emoji;
  }

  // 🌡️ TONE SYNC
  if (hour >= 15 && future?.length > 1) {
    const t0 = future[0].temp;
    const t1 = future[future.length - 1].temp;
  ...
  const todayModule = document.getElementById("today-module");
  if (todayModule) {
    todayModule.classList.toggle("fade", hour >= 15);
  }
  ...
  // 🔥 FINAL LABEL CONTROL (THIS FIXES YOUR ISSUE)
  if (labelEl) {
    labelEl.textContent =
      hour >= 15
        ? "Tonight’s Human-Action Outlook"
...
export function renderTomorrowOutlook(intel) {
  ...
  const data = buildTomorrowCurrent(tomorrow.stats);
  const action = generateHumanAction(data);
